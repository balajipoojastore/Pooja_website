...
export default function CheckoutPage() {
  ...
  return <div className="checkout-page shell"><header className="page-heading"><span className="eyebrow">Secure checkout</span><h1>Review and place your order</h1><p>Signed in as {auth.user?.email}. Prices and availability are checked again on the server.</p></header><div className="checkout-layout"><div className="checkout-form">
    <section><h2><span>1</span>Customer</h2><div className="checkout-identity"><strong>{auth.profile?.full_name}</strong><p>{auth.profile?.phone} · format validated</p><small>Email verified by Supabase Auth</small></div></section>
    ...
    <button className="primary-btn place-order" type="button" onClick={() => void placeOrder()} disabled={submitting || !termsAccepted}>{submitting ? <><LoaderCircle className="spin" />Placing order securely…</> : <><LockKeyhole />Place COD order · {formatINR(totals.totalPaise)}</>}</button>
